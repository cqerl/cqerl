...
insert() ->
    receive
        {Pid, Client, Tag, N} ->
            case cqerl:run_query(Client, #cql_query{statement = list_to_binary("INSERT INTO userlocation(fjid, ip) VALUES('keyprefix"++ integer_to_list(N) ++"','defaultvaluehere');"), consistency = 1}) of
                {ok, void} ->
                    Pid ! {result, Tag},
                    ok;
                Fail ->
