...
encode_query_parameters(#cqerl_query_parameters{consistency=Consistency,
  ...
  Flags2 = Flags1 + case SkipMetadata of
    true -> 1 bsl 1;
    _ -> 0
  end,
...
decode_result_row(ColumnCount, Binary, Acc) ->
  {ok, CellValueBin, Rest} = ?DATA:decode_bytes(Binary),
  decode_result_row(ColumnCount-1, Rest, [CellValueBin|Acc]).
...
-spec batch_frame(RequestFrame :: #cqerl_frame{}, BatchParameters :: #cqerl_query_batch_parameters{}) -> 
  {ok, binary()} | {error, badarg}.

batch_frame(Frame=#cqerl_frame{}, #cqerl_query_batch_parameters{consistency=Consistency,
                                                                mode=Mode,
                                                                queries=Queries}) ->
...
decode_response_term(#cqerl_frame{opcode=?CQERL_OP_RESULT}, << 2:?INT, Body/binary >>) ->
  ...
  {ok, {Metadata#cqerl_result_metadata{rows_count=RowsCount}, ResultSet}};
...
decode_response_term(#cqerl_frame{opcode=?CQERL_OP_RESULT}, << 4:?INT, Body/binary >>) ->
  ...
  {ok, {QueryID, QueryMetadata, ResultMetadata}};
...
decode_response_term(#cqerl_frame{opcode=?CQERL_OP_RESULT}, << 5:?INT, Body/binary >>) ->
  ...
  ChangeType = binary_to_atom(ChangeName, latin1),
  {ok, KeySpaceName, Rest2} = ?DATA:decode_string(Rest1),
  {ok, TableName, _Rest} = ?DATA:decode_string(Rest2),
  {ok, {ChangeType, KeySpaceName, TableName}};
...
decode_response_term(#cqerl_frame{opcode=?CQERL_OP_EVENT}, Body) ->
  ...
  case EventName of
    _ when EventName == ?CQERL_EVENT_TOPOLOGY_CHANGE;
